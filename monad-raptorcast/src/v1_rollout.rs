...
use crate::{
    udp::ValidatedChunk,
    util::{BroadcastMode, BuildTarget, PrimaryBroadcastGroup},
};
...
pub(crate) fn should_accept<PT: PubKey>(
    ...
    match chunk.broadcast_mode {
        BroadcastMode::DeterministicPrimary(_) => should_accept_v1(stage),
        BroadcastMode::Primary => should_accept_v0(stage),
        _ => panic!("should_accept called on non-primary raptorcast chunk"),
    }
}
