...
use crate::{
    ...
    util::{ensure, BroadcastMode, Collector, PrimaryBroadcastGroup, Redundancy, UdpMessage},
    SIGNATURE_SIZE,
};
...
impl PacketLayout {
    ...
    pub const MIN_CHUNK_LEN: usize = PacketLayout::SEGMENT_LEN - PacketLayout::MAX_HEADER_LEN;
    pub const MAX_CHUNK_LEN: usize = PacketLayout::SEGMENT_LEN - PacketLayout::MIN_HEADER_LEN;

    // SAFETY: the caller must ensure the asserted preconditions hold
    pub fn new(app_message_len: usize, num_validators: usize) -> Self {
        debug_assert!(app_message_len > 0 && app_message_len <= MAX_MESSAGE_SIZE);
    ...
    pub fn optimal_tree_depth(app_message_len: usize, num_validators: usize) -> u8 {
        ...
        for depth in MIN_MERKLE_TREE_DEPTH..=MAX_MERKLE_TREE_DEPTH {
            ...
            debug_assert!((Self::MIN_CHUNK_LEN..=Self::MAX_CHUNK_LEN).contains(&symbol_len));
    ...
    // The number of chunks is structured as:
    //
    //   K = num_source_symbols
    //   R*K = num_repair_symbols
    //   V = num_validators
    //
    // Total chunks produced by the assigner: <= R*K + V
    // Upper bound used for merkle tree sizing: R*K + V

    // K: the minimum number of encoding symbols needed by the
    // raptor10 decoder to reconstruct the app message.
    pub fn num_source_symbols(&self) -> usize {
        self.app_message_len.div_ceil(self.symbol_len()).max(1)
    }

    // R*K: the number of encoding symbols to distribute across
    // validators via the chunk assigner. Each validator receives a
    // stake-proportional share of these symbols.
    pub fn num_encoded_symbols(&self) -> usize {
        // SAFETY: verified in tests::test_no_panic_on_valid_ranges
        REDUNDANCY
            .scale(self.num_source_symbols())
            .expect("scale overflow")
    }

    // R*K + V: upper bound on total chunks, used for merkle tree
    // leaf allocation and decoder capacity. The extra V comes from
    // the stake-based assigner rounding up each validator's share.
    pub fn encoded_symbol_capacity(&self) -> usize {
        self.num_encoded_symbols() + self.num_validators
    }

    pub const fn symbol_len(&self) -> usize {
        let symbol_len =
            Self::SEGMENT_LEN - Self::HEADER_LEN - self.merkle_proof_len() - Self::CHUNK_HEADER_LEN;
        debug_assert!(symbol_len >= Self::MIN_CHUNK_LEN && symbol_len <= Self::MAX_CHUNK_LEN);
        symbol_len
    }
...
fn make_chunks<PT: PubKey>(
    group: &PrimaryBroadcastGroup<'_, PT>,
    app_message: &[u8],
    round: monad_types::Round,
    unix_ts_ms: u64,
) -> Result<(Vec<Chunk<PT>>, PacketLayout), BuildError> {
    ensure!(
        !app_message.is_empty() && app_message.len() <= MAX_MESSAGE_SIZE,
        BuildError::AppMessageTooLarge
    );

    // Step 1: derive a seed
    let author = group.author();
    let seed = derive_seed(author, round, unix_ts_ms);

    // Step 2: shuffle the validator set based on the seed
    ...
    // Step 3: assign chunks to validators based on the shuffled validator set
    let layout = PacketLayout::new(app_message.len(), validators.len());
    let assigner = StakeBasedWithRC::from_validator_set(validators);
    let assignment = assigner.assign_chunks(layout.num_encoded_symbols(), None)?;
    let chunks = assignment.generate(PacketLayout::SEGMENT_LEN);
    ...
    C: Collector<UdpMessage<PT>>,
{
    ...
    let (mut chunks, layout) = make_chunks(group, app_message, round, unix_ts_ms)?;
...
mod tests {
    ...
    const _: () = assert!(
        PacketLayout::MIN_CHUNK_LEN >= crate::packet::regular::MIN_CHUNK_LENGTH,
        "MIN_CHUNK_LEN must be no smaller than regular MIN_CHUNK_LENGTH"
    );
    ...
    const _: () = assert!(PacketLayout::MIN_CHUNK_LEN == 1020);
    const _: () = assert!(PacketLayout::MAX_CHUNK_LEN == 1260);

    fn validate_layout(app_msg_len: usize, val_set_size: usize) {
        // no panicking
        let layout = PacketLayout::new(app_msg_len, val_set_size);
        let _ = layout.num_source_symbols();
