...
use crate::{
    ...
    util::{
        ...
        FullNodeGroupMap, GlobalMerkleRoot, MerkleRoot, NodeIdHash, PrimaryBroadcastGroup,
        SecondaryBroadcastGroup,
    },
    ...
    pub fn handle_message(
        ...
        for payload_start_idx in (0..message.payload.len()).step_by(message.stride.into()) {
            ...
            let maybe_decoded_message = match chunk.broadcast_mode {
                ...
                BroadcastMode::Primary | BroadcastMode::DeterministicPrimary(_) => {
                    if !v1_rollout::should_accept(self.v1_rollout, &chunk) {
                        self.metrics.executor_metrics_mut()
                            [COUNTER_RAPTORCAST_CHUNKS_DROPPED_INCOMPATIBLE_VERSION] += 1;
                        continue;
                    }
                    let accepted_metric = match chunk.broadcast_mode {
                        BroadcastMode::Primary => COUNTER_RAPTORCAST_V0_PRIMARY_CHUNKS_ACCEPTED,
                        _ => COUNTER_RAPTORCAST_V1_PRIMARY_CHUNKS_ACCEPTED,
                    };
...
impl<PT: PubKey> ValidatedChunk<PT> {
    ...
    pub fn global_merkle_root(&self) -> Option<&GlobalMerkleRoot> {
        match self.broadcast_mode {
            BroadcastMode::DeterministicPrimary(_) => Some(&self.merkle_root),
            _ => None,
        }
    }
    ...
    pub fn check_deterministic_encoding(
        ...
        let BroadcastMode::DeterministicPrimary(round) = self.broadcast_mode else {
            return None;
        };

        match deterministic::calc_global_merkle_root(app_message, round, self.unix_ts_ms, group) {
...
mod tests_deterministic {
    ...
    fn test_deterministic_decode_with_partial_chunks() {
        ...
        let app_message: Bytes = vec![0xAB_u8; crate::message::MAX_MESSAGE_SIZE].into();
        let packets = build_packets(&sender_key, &app_message, group);

        // With 3x redundancy we should have >> num_source_symbols chunks.
        let layout = deterministic::PacketLayout::new(app_message.len(), validators.len() - 1);
        let num_source = layout.num_source_symbols();
        assert!(
