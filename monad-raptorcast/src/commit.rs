...
use crate::{
    udp::ValidatedChunk,
    util::{BroadcastMode, GlobalMerkleRoot},
    SIGNATURE_SIZE,
};
...
const PAST_ROUND_RETENTION: Round = Round(100);
...
const FUTURE_ROUND_ALLOWANCE: Round = Round(100);
    ...
    PT: PubKey,
{
    ...
    fn try_from(chunk: &'a ValidatedChunk<PT>) -> Result<Self, ()> {
        let BroadcastMode::DeterministicPrimary(round) = chunk.broadcast_mode else {
            // not applicable
            return Err(());
        };
...
mod tests {
    ...
    fn chunk(
        ...
        ValidatedChunk {
            ...
            broadcast_mode: BroadcastMode::DeterministicPrimary(Round(round)),
            merkle_root: *merkle,
            ..dummy_chunk()
        }
    }
    ...
    fn round_window_boundaries() {
        ...
        rc.update_current_round(Round(10));
        // window is [10 - 3, 10 + 3] = [7, 13]
        assert!(rc.len() == 0); // oob commitments evicted

        assert!(rc.try_commit(&chunk(6, &SIG_A, &MERKLE_A)).is_err());
        assert!(rc.try_commit(&chunk(7, &SIG_A, &MERKLE_A)).is_ok());
        assert!(rc.try_commit(&chunk(13, &SIG_A, &MERKLE_A)).is_ok());
        assert!(rc.try_commit(&chunk(14, &SIG_A, &MERKLE_A)).is_err());

        rc.update_current_round(Round(1));
        // window is [0 (saturated), 4]
        assert!(rc.try_commit(&chunk(0, &SIG_A, &MERKLE_A)).is_ok());
        assert!(rc.try_commit(&chunk(4, &SIG_A, &MERKLE_A)).is_ok());
        assert!(rc.try_commit(&chunk(5, &SIG_A, &MERKLE_A)).is_err());
    }
