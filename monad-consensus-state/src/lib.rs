    ...
    CRT: ChainRevision,
{
    ...
    pub fn handle_timeout_expiry(
        &mut self,
        timeout_round: Round,
        _record_local_timeout_metrics: bool,
    ) -> Vec<ConsensusCommand<ST, SCT, EPT, BPT, SBT, CCT, CRT>> {
    ...
    pub fn send_vote_and_reset_timer(
        ...
        if let Some(ready_at) = self.consensus.vote_ready_at.take() {
            self.metrics
                ...
                .add(ready_at.elapsed().as_micros() as u64);
            self.metrics.consensus_timing.vote_send_wait_count.add(1);
        }
    ...
    fn try_vote(
        ...
        if let Some(start) = self.consensus.proposal_vote_start.take() {
            ...
            self.metrics.consensus_timing.vote_count.add(1);
        }
        info!(
        ...
        match high_extend {
            ...
            HighExtend::Qc(qc) => {
                ...
                self.metrics
                    ...
                    .add(exec_result_start.elapsed().as_micros() as u64);
                self.metrics
                    .consensus_timing
...
mod test {
    ...
    where
        ...
        SBT: StateBackend<ST, SCT, EPT> + MockExecution<ST, SCT>,
        BVT: BlockValidator<
            ST,
            SCT,
    ...
    where
        ...
        BPT: BlockPolicy<ST, SCT, EthExecutionProtocol, SBT, MockChainConfig, MockChainRevision>
            + std::fmt::Debug,
        SBT: StateBackend<ST, SCT, EPT> + std::fmt::Debug,
    {
        cmds.iter()
            .find_map(|c| match c {
