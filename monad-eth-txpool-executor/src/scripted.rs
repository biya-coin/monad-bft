    ...
    CRT: ChainRevision,
{
    ...
    ) -> io::Result<Option<Vec<TxEnvelope>>> {
    ...
    async fn try_read_block(stream: &mut UnixStream) -> io::Result<Option<Vec<TxEnvelope>>> {
        ...
        let max_msg_len = CHAIN_PARAMS_LATEST.proposal_byte_limit as usize;
        if msg_len > max_msg_len {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                format!("message too large: {msg_len} bytes (max {max_msg_len})"),
            ));
        }
        ...
        let txs = Vec::<TxEnvelope>::decode(&mut msg_buf.as_slice()).map_err(|e| {
