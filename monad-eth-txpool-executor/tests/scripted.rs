...
use monad_types::{
    Epoch, LimitedVec, NodeId, Round, SeqNum, GENESIS_BLOCK_ID, GENESIS_ROUND, GENESIS_SEQ_NUM,
};
...
fn create_validated_block(
    ...
    let body = ConsensusBlockBody::new(ConsensusBlockBodyInner {
        execution_body: EthBlockBody {
            transactions: LimitedVec(raw_txs),
    ...
    let (base_fee, base_trend, base_moment) =
        block_policy.compute_base_fee(extending_blocks, chain_config);
...
async fn setup_scripted_executor() -> (
    ...
    let executor =
        ScriptedTxPoolExecutorClient::start(socket_path, block_policy, state_backend, chain_config)
            .unwrap();

    let keypair = NopKeyPair::from_bytes(rand::random::<[u8; 32]>().as_mut_slice()).unwrap();

    (executor, keypair, chain_config, tmpdir)
}
...
async fn create_proposal(
    ...
    executor.exec(vec![TxPoolCommand::CreateProposal {
        ...
        tx_limit: chain_params.tx_limit,
        proposal_gas_limit: chain_params.proposal_gas_limit,
        proposal_byte_limit: chain_params.proposal_byte_limit,
        beneficiary,
        timestamp_ns,
...
async fn test_scripted_empty_block_is_coherent() {
    let (mut executor, keypair, chain_config, _tmpdir) = setup_scripted_executor().await;
    ...
    let (proposal, base_fee, _base_fee_trend, _base_fee_moment) = match event {
    ...
    assert!(base_fee > 0, "base_fee should be non-zero");
...
async fn test_scripted_sequential_blocks_are_coherent() {
    ...
    for i in 1..=5u64 {
        ...
        BlockPolicy::<
            ...
            MonadChainConfig,
            MonadChainRevision,
        >::update_committed_block(&mut block_policy_for_validation, &validated_block);
